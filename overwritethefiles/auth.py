from flask import (
    Blueprint,
    current_app,
    flash,
    jsonify,
    redirect,
    render_template,
    request,
    session,
    url_for,
)

from app.models.users import create_user, verify_user
from app.models.users import get_admin_request, get_user_by_id, request_admin_access
from app.security import csrf_token, validate_csrf


auth_bp = Blueprint("auth", __name__)


def wants_json_response():
    return request.headers.get("X-Requested-With") == "XMLHttpRequest"


@auth_bp.route("/login", methods=["GET", "POST"])
def login():
    if request.method == "POST":
        email = request.form.get("email", "")
        password = request.form.get("password", "")

        user = verify_user(current_app.config["DATABASE"], email, password)
        if user is None:
            if wants_json_response():
                return jsonify({"message": "Invalid email or password."}), 401

            flash("Invalid email or password.", "error")
            return render_template("auth/login.html", email=email), 401

        session.clear()
        session["user_id"] = user["id"]
        session["user_email"] = user["email"]
        # Use the verified database role for both normal and AJAX login redirects.
        destination = "admin.dashboard" if user["is_admin"] else "client.upload"

        if wants_json_response():
            return jsonify(
                {
                    "message": "Your login is successful.",
                    "redirect_url": url_for(destination),
                }
            )

        flash("You are logged in.", "success")
        return redirect(url_for(destination))

    return render_template("auth/login.html")


@auth_bp.route("/register", methods=["GET", "POST"])
def register():
    if request.method == "POST":
        email = request.form.get("email", "")
        password = request.form.get("password", "")

        if len(password) < 8:
            flash("Password must be at least 8 characters.", "error")
            return render_template("auth/register.html", email=email), 400

        user = create_user(current_app.config["DATABASE"], email, password)
        if user is None:
            flash("An account with that email already exists.", "error")
            return render_template("auth/register.html", email=email), 409

        session.clear()
        session["user_id"] = user["id"]
        session["user_email"] = user["email"]
        flash("Account created. You are logged in.", "success")
        return redirect(url_for("client.upload"))

    return render_template("auth/register.html")


@auth_bp.route("/admin-access", methods=["GET", "POST"])
def admin_access():
    # Requests belong to the signed-in account; never accept a user ID from the form.
    user = get_user_by_id(current_app.config["DATABASE"], session.get("user_id"))
    if user is None:
        return redirect(url_for("auth.login"))
    if user["is_admin"]:
        return redirect(url_for("admin.dashboard"))

    error = None
    reason = ""
    if request.method == "POST":
        validate_csrf()
        reason = request.form.get("reason", "").strip()
        # Enforce the browser's length rules on the server as well.
        if not 10 <= len(reason) <= 1000:
            error = "Enter a reason between 10 and 1,000 characters."
        else:
            request_admin_access(current_app.config["DATABASE"], user["id"], reason)
            return redirect(url_for("auth.admin_access"), code=303)

    # An existing request displays its saved status instead of another request form.
    return render_template(
        "auth/admin_access.html",
        access_request=get_admin_request(current_app.config["DATABASE"], user["id"]),
        csrf_token=csrf_token(), error=error, reason=reason,
        active_page="admin-access", username=user["email"],
    ), 400 if error else 200


@auth_bp.route("/dashboard")
def dashboard():
    if "user_id" not in session:
        flash("Please log in first.", "error")
        return redirect(url_for("auth.login"))

    return render_template("auth/dashboard.html", email=session["user_email"])


@auth_bp.route("/logout")
def logout():
    session.clear()
    flash("You are logged out.", "success")
    return redirect(url_for("auth.login"))
